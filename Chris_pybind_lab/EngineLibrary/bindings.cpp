...
public:
    ...
    // Draw the outline of a simple rectangle
    void DrawRectangle(int x, int y, int w, int h, int r, int g, int b, int a);
    // Draw a filled-in simple rectangle
    void FillRectangle(int x, int y, int w, int h, int r, int g, int b, int a);
    // Poll for events, retunring an int coressponding with the event that happend.
    int PollEvent();
...
void SDLGraphicsProgram::DrawRectangle(int x, int y, int w, int h, int r, int g, int b, int a){
    SDL_Rect fillRect = {x,y,w,h};
    SDL_SetRenderDrawColor(gRenderer, r, g, b, a);
    SDL_RenderDrawRect(gRenderer, &fillRect); 
}

void SDLGraphicsProgram::FillRectangle(int x, int y, int w, int h, int r, int g, int b, int a){
    SDL_Rect fillRect = {x,y,w,h};
    SDL_SetRenderDrawColor(gRenderer, r, g, b, a);
    SDL_RenderFillRect(gRenderer, &fillRect); 
}

int SDLGraphicsProgram::PollEvent() {
    SDL_Event e;
    // Enable text input
    SDL_StartTextInput();
    //Handle events on queue
...
PYBIND11_MODULE(mygameengine, m){
    ...
    py::class_<SDLGraphicsProgram>(m, "SDLGraphicsProgram")
            ...
            .def("FillRectangle", &SDLGraphicsProgram::FillRectangle)
            .def("PollEvent", &SDLGraphicsProgram::PollEvent) ;
